...
const subjectData = [
  {
    ...
    formats: ['online'],
    pathways: ['Research Scholar Programs', 'Intensive Programs', 'Career Exploration and University Pathways'],
  },
  {
    name: 'Medicine',
    ...
    formats: ['online'],
    pathways: ['Research Scholar Programs', 'Intensive Programs', 'Leadership and Personal Development'],
  },
  {
    name: 'Mathematics',
...
const FindACourse = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-25">
      ...
      <section className="py-4 sm:py-6 md:py-12 lg:py-16 xl:py-20 bg-gradient-to-br from-[#0F2A44] to-[#1a3a5a]">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-semibold text-white mb-2 sm:mb-3 md:mb-4">Find a Research Online Program</h1>
          {/* Description hidden on mobile, visible on larger screens */}
          <p className="hidden sm:block text-sm sm:text-base md:text-lg lg:text-xl text-gray-100 max-w-2xl font-medium">
            Explore our diverse range of research-focused online programs and discover the perfect fit for your academic journey. All our programs are delivered online with expert mentorship from top universities.
          </p>
        </div>
      </section>
      ...
      <section className="py-4 sm:py-6 md:py-8 lg:py-12 bg-white">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
          ...
          {/* Mobile Search Bar - Moved up with reduced spacing */}
          <div className="block lg:hidden mb-3 sm:mb-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#B8A67A] focus:border-[#B8A67A] outline-none transition-all duration-200 placeholder-gray-500 shadow-sm"
              />
              {searchQuery && (
                <button
                  onClick={clearSearch}
          ...
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 md:gap-8 lg:gap-10">
            ...
            {/* Filter Sidebar */}
            <aside className={`
              w-full lg:w-80 xl:w-96 bg-gray-50 rounded-lg lg:rounded-xl shadow-sm lg:shadow-md p-4 sm:p-5 lg:p-6
              ${showFilters ? 'block' : 'hidden'} lg:block
              lg:sticky lg:top-24 lg:h-fit lg:z-10
            `}>
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h2 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900">Filter Courses</h2>
                {activeFiltersCount > 0 && (
                  <button
                    onClick={clearAllFilters}
              ...
              {/* Teaching Format Filter */}
              <div className="mb-4 sm:mb-6">
                <h3 className="text-sm sm:text-base lg:text-lg font-medium text-gray-800 mb-3">Teaching Format</h3>
                <ul className="space-y-2 sm:space-y-3">
                  {teachingFormats.map((format) => (
                    <li key={format.value} className="flex items-center">
                      ...
                      />
                      <span
                        className="w-4 h-4 sm:w-5 sm:h-5 border-2 border-gray-400 rounded-sm mr-2 sm:mr-3 flex items-center justify-center peer-checked:bg-[#0F2A44] peer-checked:border-[#0F2A44] transition-colors duration-150 cursor-pointer"
                        tabIndex={0}
                        role="checkbox"
                      ...
                      <label htmlFor={format.value} className="text-gray-700 select-none cursor-pointer text-xs sm:text-sm lg:text-base">
                        {format.label}
                      </label>
                    </li>
                  ))}
              ...
              {/* Pathways Filter */}
              <div className="mb-4 sm:mb-6">
                <h3 className="text-sm sm:text-base lg:text-lg font-medium text-gray-800 mb-3">Pathways</h3>
                <ul className="space-y-2 sm:space-y-3">
                  {pathways.map((path) => (
                    <li key={path} className="flex items-start">
                      ...
                      />
                      <span
                        className="w-4 h-4 sm:w-5 sm:h-5 border-2 border-gray-400 rounded-sm mr-2 sm:mr-3 mt-0.5 flex items-center justify-center peer-checked:bg-[#0F2A44] peer-checked:border-[#0F2A44] transition-colors duration-150 cursor-pointer shrink-0"
                        tabIndex={0}
                        role="checkbox"
                      ...
                      <label htmlFor={path} className="text-gray-700 select-none cursor-pointer text-xs sm:text-sm lg:text-base leading-tight">
                        {path}
                      </label>
                    </li>
                  ))}
              ...
              {/* Subjects Filter */}
              <div>
                <h3 className="text-sm sm:text-base lg:text-lg font-medium text-gray-800 mb-3">Subjects</h3>
                <ul className="space-y-2 sm:space-y-3 max-h-40 sm:max-h-48 lg:max-h-72 overflow-y-auto pr-2">
                  {subjectData.map((subject) => (
                    <li key={subject.name} className="flex items-start">
                      ...
                      <label htmlFor={subject.name} className="text-gray-700 select-none cursor-pointer text-xs sm:text-sm lg:text-base leading-tight">
                        {subject.name}
                      </label>
                    </li>
                  ))}
            ...
            <main className="flex-1 min-w-0 relative">
              {/* Desktop Search Bar - Only visible on desktop, positioned above the cards */}
              <div className="hidden lg:block absolute -top-16 xl:-top-20 left-0 right-0 z-20">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 sm:pl-12 pr-10 sm:pr-12 py-3 sm:py-4 text-sm sm:text-base bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#B8A67A] focus:border-[#B8A67A] outline-none transition-all duration-200 placeholder-gray-500 shadow-sm"
                  />
                  {searchQuery && (
                    <button
                      onClick={clearSearch}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
                {filteredSubjects.length === 0 ? (
                  <div className="col-span-full text-center text-gray-500 py-8 sm:py-12 md:py-16 lg:py-20">
                    <svg className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 text-gray-300 mx-auto mb-3 sm:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  filteredSubjects.map((subject) => (
                    <div key={subject.name} className="bg-white border border-gray-100 rounded-lg sm:rounded-xl lg:rounded-2xl shadow hover:shadow-lg transition-all duration-300 p-3 sm:p-4 md:p-5 lg:p-6 flex flex-col">
                      <div className="flex items-start mb-2 sm:mb-3 md:mb-4">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 bg-[#0F2A44] rounded-full flex items-center justify-center mr-2 sm:mr-3 md:mr-4 shrink-0">
                          {/* Academic book icon */}
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-900 leading-tight mb-1">
                            {subject.name}
                          </h3>
                        </div>
                      </div>
                      
                      <p className="text-gray-700 text-xs sm:text-sm md:text-base mb-3 sm:mb-4 md:mb-5 flex-1 leading-relaxed">
                        {subject.overview}
                      </p>
                      
                      <div className="flex flex-wrap gap-1 sm:gap-1.5 md:gap-2 mt-auto">
                        <span className="inline-block bg-[#0F2A44] text-white text-xs font-semibold px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 rounded-full whitespace-nowrap">
                          Subject
                        </span>
                        <span className="inline-block bg-[#B8A67A] text-white text-xs font-semibold px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 rounded-full whitespace-nowrap">
                          Online
                        </span>
                      </div>
                    </div>
